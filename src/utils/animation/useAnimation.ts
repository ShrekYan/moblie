...
export function easeOutCubic(
    ...
    return (
        amountOfChange * ((elapsed = elapsed / duration - 1) * elapsed * elapsed + 1) + initialValue
    );
}
...
export function easeOutQuart(
    ...
    return (
        -amountOfChange * ((elapsed = elapsed / duration - 1) * elapsed * elapsed * elapsed - 1) +
        initialValue
    );
}
...
export function easeInQuint(
    ...
    return (
        amountOfChange * (elapsed /= duration) * elapsed * elapsed * elapsed * elapsed +
        initialValue
    );
}
...
export function easeOutQuint(
    ...
    return (
        amountOfChange *
            ((elapsed = elapsed / duration - 1) * elapsed * elapsed * elapsed * elapsed + 1) +
        initialValue
    );
}
...
export function easeInOutQuint(
    ...
    if ((elapsed /= duration / 2) < 1) {
        return (
            (amountOfChange / 2) * elapsed * elapsed * elapsed * elapsed * elapsed + initialValue
        );
    }
    return (
        (amountOfChange / 2) * ((elapsed -= 2) * elapsed * elapsed * elapsed * elapsed + 2) +
        initialValue
    );
}
...
export function easeOutCirc(
    ...
    return (
        amountOfChange * Math.sqrt(1 - (elapsed = elapsed / duration - 1) * elapsed) + initialValue
    );
}
...
export function easeInBack(
    ...
    return (
        amountOfChange * (elapsed /= duration) * elapsed * ((s + 1) * elapsed - s) + initialValue
    );
}
...
export function easeOutBack(
    ...
    return (
        amountOfChange *
            ((elapsed = elapsed / duration - 1) * elapsed * ((s + 1) * elapsed + s) + 1) +
        initialValue
    );
}
...
export function easeInOutBack(
    ...
    if ((elapsed /= duration / 2) < 1) {
        return (
            (amountOfChange / 2) * (elapsed * elapsed * (((s *= 1.525) + 1) * elapsed - s)) +
            initialValue
        );
    }
    return (
        (amountOfChange / 2) * ((elapsed -= 2) * elapsed * (((s *= 1.525) + 1) * elapsed + s) + 2) +
        initialValue
    );
}
...
export function easeOutBounce(
    ...
    if ((elapsed /= duration) < 1 / 2.75) {
        ...
        return (
            amountOfChange * (7.5625 * (elapsed -= 2.25 / 2.75) * elapsed + 0.9375) + initialValue
        );
    } else {
        return (
            amountOfChange * (7.5625 * (elapsed -= 2.625 / 2.75) * elapsed + 0.984375) +
            initialValue
        );
    }
}
...
const animationMap: Record<string, EasingFunction> = {
    easeInQuad,
    easeOutQuad,
    easeInOutQuad,
    easeInCubic,
...
type EasingType = keyof typeof animationMap;
...
interface EasingParams {
    ...
    completeCallback?: (value: number) => void;
}

const easingFunction = (params: EasingParams = { endValue: 0 }) => {
    ...
    return function task() {
        ...
        if (elapsed < duration) {
            window.requestAnimationFrame(() => {
                progressCallback?.(
                    animationMap[animationType](elapsed, startValue, endValue, duration)
                );
                task();
            });
        } else {
...
export const useAnimation = () => {
    const memoizedEasing = useCallback(easingFunction, []);
    return memoizedEasing;
};

export const animate = (params: EasingParams) => {
    return easingFunction(params);
};
